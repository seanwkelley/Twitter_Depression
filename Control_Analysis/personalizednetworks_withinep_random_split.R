#random split of within-subject data - equal number of days within/outside a depressive episode 
...
dfl <- list()
for(i in 1:1000) {
  network_variables1 <- sample(colnames(FYP_df)[2:86],10)  
  dfl[[i]] <- (network_variables1)
}
...
for(i in 104:105) {
  ...
  for(id in id_variables){
    ...
    nrow.df <- sample(nrow(en_var),round(dim(en_var)[1]*0.5))
    
    depression_network <- en_var[nrow.df,] %>% select(-Depressed_today)
    nondepression_network <- en_var[-nrow.df,] %>% select(-Depressed_today)
  ...
  write.table(t(model1_coeff),file = "Data/Results/all_tweets/model1_coeff.csv",append = T,row.names = F,col.names = F,sep=",")
  write.table(t(model2_coeff),file = "Data/Results/all_tweets/model2_coeff.csv",append = T,row.names = F,col.names = F,sep=",")
