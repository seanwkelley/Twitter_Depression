...
ct <- 10 #interval to compute variance over   
...
depression <- list(); nondepression <- list()


for(handle in id_list) {
  print(handle)
  
  ex1 <- FYP_df_subset %>% filter(Id == handle ) %>% filter(Date != "")
  ex1$Depressed_today <- as.numeric(as.character(ex1$Depressed_today))
  mod <- depmix(WC ~ 1, data = ex1, nstates = 2, family = gaussian())
  fit.mod <- suppressWarnings(fit(mod))
  est.states <- posterior(fit.mod)
  mean.state <- as.data.frame(cbind(est.states$S2,ex1$Depressed_today))
  colnames(mean.state) <- c("Prob","Depression")
  
  dep.state <- mean((mean.state %>% filter(Depression == 1))$Prob)
  nondep.state <- mean((mean.state %>% filter(Depression == 0))$Prob)
  
  depression <- c(depression,dep.state)
  nondepression <- c(nondepression,nondep.state)
  
  
}
mean(unlist(depression) - unlist(nondepression))
hist(unlist(depression) - unlist(nondepression))
