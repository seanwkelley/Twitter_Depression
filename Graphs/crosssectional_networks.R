...
raincloud_theme = theme(
  ...
  axis.text.x = element_text(vjust = 0.5,colour = "black",angle = 0,size=20),
...
cross.sectional.plot <- plot(N1_full_stab,statistics = c("closeness","betweenness","strength"))
corStability(N1_full_stab)

#largest positive edges 
negemo_swear <- Results1$bootTable %>% filter(node1 == 'negemo' & node2 == 'swear')
i_swear <- Results1$bootTable %>% filter(node1 == 'i' & node2 == 'swear')
negemo_negate <- Results1$bootTable %>% filter(node1 == 'negemo' & node2 == 'negate')

mean(negemo_swear$value); quantile(negemo_swear$value,c(0.025,0.975))
mean(i_swear$value); quantile(i_swear$value,c(0.025,0.975))
mean(negemo_negate$value); quantile(negemo_negate$value,c(0.025,0.975))


#largest negative edges 
i_we <- Results1$bootTable %>% filter(node1 == 'i' & node2 == 'we')
i_article <- Results1$bootTable %>% filter(node1 == 'i' & node2 == 'article')
you_article <- Results1$bootTable %>% filter(node1 == 'you' & node2 == 'article')

mean(i_we$value); quantile(i_we$value,c(0.025,0.975))
mean(i_article$value); quantile(i_article$value,c(0.025,0.975))
mean(you_article$value); quantile(you_article$value,c(0.025,0.975))

#-------------------------------------------------------------------------


network <- as.data.frame(scale(centrality(dechoud_net)$InDegree))
colnames(network) <- "Strength"

network.close <- as.data.frame(scale(centrality(dechoud_net)$Closeness))
colnames(network.close) <- "Closeness"


network.between <- as.data.frame(scale(centrality(dechoud_net)$Betweenness))
...
cross.sectional.data <- cross.sectional.plot$data
ggplot(cross.sectional.data,aes(x = nPerson/max(nPerson),y=mean,group = type)) + 
  geom_line(aes(y = mean, color =type),size=1) + 
  scale_x_continuous(labels = scales::percent_format(accuracy = 1),
  trans = "reverse",breaks = c(1,0.9,0.8,0.7,0.6,0.5,0.4,0.3)) +
  cs.stability + xlab("\nSampled People") + 
  ylab("Average Correlation with Original Sample\n") + ylim(-1,1) +
...
h2 <- ggplot(data = FYP, aes(y = Depression_zscore, x = as.factor(Dep_ep_pastyear), 
