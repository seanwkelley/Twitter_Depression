...
colnames(nodep_net )[10:12] <- c("Depressive_Episode_pastyear","SDS_Total","Depressed_Today")
nodep_net <- as.data.frame(nodep_net)
nodep_net$Mean_Centrality <- rowMeans(nodep_net[,1:9])
nodep_net <- nodep_net %>% select(colnames(nodep_net)[1:9],Mean_Centrality,Depressive_Episode_pastyear,SDS_Total,Depressed_Today)
nodep_net$Id <- rownames(nodep_net)

dep_net <- do.call(rbind, depression_centrality)
colnames(dep_net )[10:12] <- c("Depressive_Episode_pastyear","SDS_Total","Depressed_Today")
dep_net <- as.data.frame(dep_net)
dep_net$Mean_Centrality <- rowMeans(dep_net[,1:9])
dep_net <- dep_net %>% select(colnames(dep_net)[1:9],Mean_Centrality,Depressive_Episode_pastyear,SDS_Total,Depressed_Today)
